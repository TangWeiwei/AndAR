...
public class CloudClient {
	 ...
	 */
	public synchronized static void init(Context context, String appname,
			String ak, String sk) throws CloudServiceException {
		if (!checkNetwork(context)) {// 网络不通直接抛异常
			throw new CloudServiceException("Network Error",
					CloudServiceException.NETWORK_ERROR);
		}
		...
		// token为空则生成token
		// if (null==token) //我去掉的
		// 获取token成功后 120秒内不再获取
		if ((System.currentTimeMillis() - tokenTime) > 1000 * 120) { // 原来是三十秒
			obtainToken();
		}
	}
	...
	private static void initHttpClient() throws CloudServiceException {
		if (httpclient == null) {
			...
			try {
				...
				errorMessage = "CloudClient.initHttpClient() error! IOException:"
						+ e.getMessage();
			}
	...
	public static JsonObject post(String url, Map<String, String> params,
		...
		if (null != params && params.size() > 0) {
			...
			try {
				...
				throw new CloudServiceException("CloudClient.post(" + url + ","
						+ params + ") Error! UnsupportedEncodingException:"
						+ e.getMessage(), CloudServiceException.CLIENT_ERROR);
			}
		}
	...
	public static JsonObject get(String url, Map<String, String> params,
		...
		if (null != params && params.size() > 0) {
			...
			try {
				...
				throw new CloudServiceException("CloudClient.get(" + url + ","
						+ params + ") Error! UnsupportedEncodingException:"
						+ e.getMessage(), CloudServiceException.CLIENT_ERROR);
			}
	...
	public static JsonObject delete(String url, Map<String, String> params,
		...
		if (null != params && params.size() > 0) {
			...
			try {
				...
				throw new CloudServiceException("CloudClient.delete(" + url
						+ "," + params
	 ...
	 */
	public static JsonObject upload(String url, String filepath,
			String cloudpath, ActionCallback callback)
			throws CloudServiceException {
		File file = new File(filepath);
		if (!file.exists()) {// 验证本地文件是否存在
			callback.done(null, new CloudServiceException("CloudClient.upload("
					+ url + "," + filepath + "," + cloudpath
	 ...
	 * @return 返回服务端数据的json对象
	 */
	private static JsonObject service(final HttpUriRequest request,
			final ActionCallback callback) throws CloudServiceException {
	 ...
	 * @return 返回服务端数据的json对象
	 */
	private static JsonObject internalService(HttpUriRequest request,
			ActionCallback callback) throws CloudServiceException {
		if (!checkNetwork(context)) {// 网络不通直接抛异常
			throw new CloudServiceException("Network Error",
					CloudServiceException.NETWORK_ERROR);
		...
		try {
			...
			if (200 == httpCode) {
				...
				errorMessage = "CloudClient.internalService("
						+ request
						+ ","
						+ callback
						+ ") Error!errorMessage:HTTP Reqeust Error!HTTP Code is "
						+ httpCode;// http请求错误
			}
		} catch (ClientProtocolException e) {
			errorMessage = "CloudClient.internalService(" + request + ","
					+ callback
					+ ") Error!errorMessage: ClientProtocolException "
					+ e.getMessage();
		} catch (ParseException e) {
			errorMessage = "CloudClient.internalService(" + request + ","
					+ callback + ") Error!errorMessage:ParseException "
					+ e.getMessage();
		} catch (IOException e) {
			errorMessage = "CloudClient.internalService(" + request + ","
					+ callback + ") Error!errorMessage:IOException "
					+ e.getMessage();
		}
		if (null == callback) {
			return json;
			...
			if (errorMessage != null) {
				Log.e("CloudService", errorMessage);
				e = new CloudServiceException(errorMessage,
						CloudServiceException.CLIENT_ERROR);
			}
	 ...
	 */
	public static synchronized String obtainToken()
			throws CloudServiceException {
		String errorMessage = null;
		// 获取token成功后 120秒内不再获取
		// if ((System.currentTimeMillis()-tokenTime)>1000*120) { //原来是三十秒
		...
		if (ret != null) {
			...
			if (code == 0 && data != null) {
				...
				tokenTime = System.currentTimeMillis(); // 我加的
				return token;
			} else {
				errorMessage = "CloudClient.obtainToken() Error! Code is:"
						+ code + " Message is:"
						+ ret.get("message").getAsString();// 获取token失败
			}
		} else {
		...
		// }else{
		// errorMessage =
		// "CloudClient.obtainToken() Error! Request Token Too Often!Recently Time is "+new
		// Date(tokenTime);//获取token时间太频繁
		// }
		Log.e("CloudService", errorMessage);
		throw new CloudServiceException(errorMessage,
				CloudServiceException.CLIENT_ERROR);
